...
export const exportToCSV = async (transactions, categories, startDate, endDate) => {
  try {
...
export const exportToTextReport = async (transactions, categories, startDate, endDate) => {
  try {
    // Filter by date range if provided
    let filteredTransactions = transactions;
    if (startDate && endDate) {
    ...
    sorted.forEach(transaction => {
      ...
      if (transaction.notes) {
        report += `Catatan: ${transaction.notes}\n`;
      }
      report += '\n';
    });
...
export const exportSummaryReport = async (transactions, categories) => {
  try {
