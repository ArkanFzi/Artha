...
export const filterTransactionsByDateRange = (transactions, startDate, endDate) => {
  return transactions.filter(t => {
    try {
      ...
      const startTimestamp = startDate.getTime ? startDate.getTime() : Date.parse(startDate);
      const endTimestamp = endDate.getTime ? endDate.getTime() : Date.parse(endDate);
...
export const calculateTotalIncome = (transactions) => {
  return transactions
    ...
    .reduce((sum, t) => sum + (parseFloat(t.amount) * (parseFloat(t.exchangeRate) || 1)), 0);
};
...
export const groupExpensesByCategory = (transactions, categories) => {
  ...
  expenses.forEach(transaction => {
    ...
    grouped[categoryId].total += (parseFloat(transaction.amount) * (parseFloat(transaction.exchangeRate) || 1));
...
export const getMonthlySpendingTrend = (transactions, year) => {
  ...
  yearTransactions.forEach(t => {
    if (t.type === 'expense') {
      try {
        const dateStr = t.date || t.createdAt;
        const month = parseInt(dateStr.slice(5, 7)) - 1; // Extract month from YYYY-MM-DD
        if (month >= 0 && month < 12) {
          monthlyData[month] += (parseFloat(t.amount) * (parseFloat(t.exchangeRate) || 1));
        }
      } catch (error) {
...
export const calculateInvestmentRecommendation = (totalIncome, totalExpenseBudget) => {
  const remainingBudget = totalIncome - totalExpenseBudget;
  const percentage = (remainingBudget / totalIncome) * 100;
  
  // Recommended investment: 10-30% of income
...
export const getInvestmentSuggestions = (remainingPercentage) => {
