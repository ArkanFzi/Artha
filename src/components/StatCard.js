...
const StatCard = ({ 
  ...
  value, 
  icon, 
  gradientColors, // Not used heavily anymore, but kept for compatibility
  onPress,
  subtitle,
  size = 'medium',
  layout = 'vertical'
  ...
  // Modern soft background instead of heavy gradient
  ...
  const getSizeStyles = () => {
    switch (size) {
      case 'small':
        return { iconSize: 18, valueSize: 18, titleSize: 11, padding: 12 };
      case 'large':
        return { iconSize: 28, valueSize: 28, titleSize: 14, padding: 18 };
      case 'full':
        return { iconSize: 24, valueSize: 24, titleSize: 13, padding: 16, fullWidth: true };
      default: // medium
        return { iconSize: 20, valueSize: 22, titleSize: 12, padding: 14 };
    }
  };
      ...
      {layout === 'vertical' ? (
        <>
          <View style={styles.header}>
            ...
            {/* Optional decoration dot */}
            <View style={[styles.dot, { backgroundColor: gradientColors?.[0] || theme.primary }]} />
          </View>
...
const styles = StyleSheet.create({
  ...
  card: {
    ...
    padding: 14,
    borderWidth: 1,
    // Removed justifyContent: 'space-between' to make it more compact
  },
  cardHorizontal: {
    flexDirection: 'row',
    alignItems: 'center',
